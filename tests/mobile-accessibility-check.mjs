...
const buttons = [...markup.matchAll(/<button\b[^>]*>/gi)].map((match) => match[0]);
...
const controls = [...markup.matchAll(/<(?:input|select|textarea)\b[^>]*>/gi)].map((match) => match[0]);
...
const dialogs = [...markup.matchAll(/<[^>]+role\s*=\s*["']dialog["'][^>]*>/gi)].map((match) => match[0]);
