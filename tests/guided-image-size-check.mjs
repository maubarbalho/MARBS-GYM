...
const required = [
  ...
  'width:76px; height:76px; min-height:76px',
  ".guided-media-visual::after { content:''; position:absolute; inset:8px;",
  '.guided-art-mark { width:38px; height:38px;'
];
for (const token of required) {
  if (!html.includes(token)) throw new Error(`A área compacta da imagem não contém: ${token}`);
}
if (html.includes('grid-template-columns:130px 1fr') || html.includes('grid-template-columns:108px 1fr') || html.includes('min-height:138px') || html.includes('min-height:104px')) {
